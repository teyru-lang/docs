...
## 4. Editor tooling (`editors/`)

| Component | Source | License |
|---|---|---|
| `editors/tree-sitter-teyru/src/tree_sitter/alloc.h`, `array.h`, `parser.h` | Copied from [tree-sitter](https://github.com/tree-sitter/tree-sitter) for use by the generated parser | MIT (Copyright (c) 2018 Max Brunsfeld) |
| `editors/tree-sitter-teyru/src/parser.c` | Generated by the tree-sitter CLI (v0.25.10) from `grammar.js`, and contains tree-sitter's runtime code | MIT (same as above); marked as generated in `.gitattributes` |
| `editors/tree-sitter-teyru/grammar.js`, `queries/`, `test/` | Original to this project | GPL-2.0-only, see `editors/tree-sitter-teyru/LICENSE` |
| `editors/vscode/` (TextMate grammar, language configuration, snippets) | Original to this project | See `LICENSE` |
...
| `editors/tree-sitter-teyru/LICENSE` | The license of the grammar itself (GPL-2.0-only, the same as this project's main license) |
