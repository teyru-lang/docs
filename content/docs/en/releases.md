...
|---|---|---|
| Executable size (hello world, `-O2`) | **66,688 B** (`-O1` 85,320, `-O3` 70,128) | `wc -c`; larger than the 55,920 of the 0.2 era, by 6,016 for the boxing caches and 2,256 for the stack check — the arithmetic is in [docs/index.md](/en/docs) |
| The LLVM back end's boundary | of 256 programs, 159 build, 153 produce the expected output, 93 refused by name | 2026-09-17, `teyru build --backend=llvm` per program, compared against each `.expected` (the full breakdown is in [docs/index.md](/en/docs)) |
