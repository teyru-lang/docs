...
| `@Tolerate` | ✅ Complete | the annotated member "does not exist" for the generator: `@Setter private Date date` plus `@Tolerate public void setDate(String)` yields two overloads |
| `@Locked` | ✅ Complete | wraps the method body with a named lock field |
| `@NonFinal` | ✅ Complete | removes final |
| `@PackagePrivate` | ✅ Complete | removes the access modifier |
...
the table above once inside a single program, with the output compared line by line;
...
- The string generated by `@ToString(callSuper = true)` is `Child(c=2; super=Base(b=1))`,
  whereas Lombok is `Child(super=Base(b=1), c=2)`: its own fields are written first, and the
  super part is last.
...
2. **`@NonNull` has no field-write interception.** When a field is marked `@NonNull` and is
   taken into a generated constructor, a check is inserted, and the setter generated by
   `@Setter` plus the parameters of hand-written methods and constructors (marking the
   parameter alone is enough) also get checks inserted; Lombok also checks "direct field
   assignment", but Teyru's field reads and writes do not go through methods, so it cannot
   intercept them. This limitation cannot be expressed with a setter in Lombok either.
...
9. **`@Helper` only recognises local classes inside a method.** There an instance is
   ...
   it; written on a member class it has no effect (Lombok is the same), it merely marks the
   class static. `@Tolerate`, on the other hand, makes the generator "unable to see" the
   annotated member: with `@Setter private Date date` plus
