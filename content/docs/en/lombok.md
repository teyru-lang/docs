...
| `@Accessors` | ✅ Complete | `chain` / `fluent` / `prefix`; `fluent = true` makes setters chainable with it, a `chain` written out still decides on its own, and a static field's setter is `void` (all as in Lombok) |
