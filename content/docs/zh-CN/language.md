...
**这不是读取程序的错，而是测试的问题（已修）。** tzdata 有两种**构建布局**，而它们对爱尔兰不一致：
**vanguard**（这台机器的 2026b）把 IST +01 当标准（夏天 `isdst=0`、冬天 GMT `isdst=1`）；
**rearguard**（Debian／Ubuntu 用它构建）正好相反——GMT 是标准（冬天 `isdst=0`、夏天 +01 的 IST
`isdst=1`）。**offset 与缩写完全相同、`isdst` 相反**，所以同一支程序在两种布局下都读对自己那个
文件，而**没有任何一个固定的期望值能同时满足两者**。

重现方式（我就是这样验的）：复制机器的 `/usr/share/zoneinfo/tzdata.zi`，把 `R IE` 那七条的 SAVE
反向、并把 `Z Europe/Dublin` 最后一段从 `1 IE IST/GMT` 改成 `0 IE GMT/IST`，各自 `zic -d out <文件>`
再用 `TZDIR` 读：2026 年三月那一笔在 vanguard 是 `GMT isdst=1`、在 rearguard 是 `GMT isdst=0`
（十月那笔相反，offset 两边都是 0 与 3600）。所以 `t189`／`t190` 改成**自己带数据**（`t190` 现在
自己写出两种布局的 negative-DST 文件），而不是去改读取程序——`lib/46_timezone.teyru` **一行未改**。

（原本写在这里的是「负 DST 时区的已知缺陷」，那个结论是错的：错的是测试，不是读者。）
...
12. **字符串以 UTF-16 code unit 为单位，与 Java 相同；存储是 WTF-8。** `length`、`charAt`、
    `substring`、`indexOf`、`compareTo`、`hashCode`、`codePoint` 家族、`toCharArray`／
    `getChars`／`chars`／`codePoints`，以及 `char[]`／`int[]` 构造函数，都按 code unit 算；
    `Character` 的分类与大小写映射用 Unicode 15.0 的数据。实测（同一支程序写两次，Java 那一半
    由 JDK 21 跑；读的 41 项里 39 项逐字相同，写的 32 项全部相同）：
    ...
    **仍然不同的两项**（刻意的，不是待办）：
    ...
    - regex 引擎逐 **code unit** 比对：`"😀a".matches(".a")` 在这里是 `false`，JDK 是 `true`
      （Java 的 `.` 吃一个 code point）；`Matcher` 回报的位移也是 code unit 索引。
    - `String.offsetByCodePoints` 走出两端时抛 `StringIndexOutOfBoundsException`，JDK 抛
      `IndexOutOfBoundsException`（前者是后者的子类，所以 `catch (IndexOutOfBoundsException)`
      仍然拦得到）。

    **存储是 WTF-8 加面包屑表**（ASCII 走快速路径，面包屑首次用到才建），所以
    `String.length()` 与 `getBytes().length` 在 U+007F 以上会分开（`"中文"` 是 `2` 与 `6`）；
    没有伙伴的代理是这个语言存得下的字符串，而 `getBytes`／`println` 把它编成一个 `?`
    （JDK 的编码器也是这样）。locale 相关的大小写映射（`tr`、`az`、`lt`）没有实现，见 §13。
...
- **Java 源代码相容的已知缺口**（`javac` 收、这里拒绝，都是实测）：**这份清单现在是空的**。
  W5 与 W7 收掉了它的每一条：`String.codePointAt`／`codePointCount`／`offsetByCodePoints`／
  ...
  结尾而每条分支都 `return` 的方法不再被误报 `TY-TYP-0020`（实测 `pick(2)` 得到 20），
  分号也不再是错误（见 §12 第 1 条）。
