...
12. **字符串以 UTF-8 字节为单位，不是 Java 的 UTF-16 code unit；字符分类与大小写映射
    只认 ASCII。** `length`、`charAt`、`substring`、`indexOf`、`compareTo`、`hashCode`
    都按字节算，所以同样的表达式在 JDK 21 与这里答案不同。实测（`teyru build` 后执行；
    JDK 21 的输出并列）：
