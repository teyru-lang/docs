...
的 stdout，**有 17 支程序在 clang 下与期望相同、在 gcc 下答案不同**。
**在 `3cd9c70`（v0.4.0 的 tag）上这条规则已经由 C 后端强制**：同一支程序在 clang 与 gcc 下都打印
`1(1)2(2)=3`（修好之前 gcc 打印的是 `2(1)1(2)`），矩阵会继续把它钉住。
**LLVM 后端有它自己的待办**：`EvalOrder` 量到 `--backend llvm -O0` 有三行与 JDK 不同；它不是
`make ci` 的腿，所以不设闸门，列为新的工作项。
