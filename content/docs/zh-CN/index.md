...
**大小那一行是成绩，而且原因是具体的。** 它是同一个 hello world 在 `-O2` 下以 `wc -c`
量的，今天 95,832 字节（约 93.6 KB）。这个数字来自编译器**剪掉没有调用点在用的
vtable 槽位**——机制写在 [docs/architecture.md](/zh-CN/docs/architecture) 的〈为什么每个
可执行文件都带着前缀〉。两天前它不是这样：`74fa648`（9/13）是 48,840 字节，之后一路
长到 501,072，因为 C 后端把每个方法都写出来、把「没人调用的东西」交给 clang 的 LTO 删，
而 LTO 删不掉地址被取用的函数——vtable 就是一串地址——于是 hello world 背着大半个标准
库（1,262 个函数存活，其中 951 个是前缀的方法，真正被调用到的只有 42 个）。

剪枝之后：hello world 从 501,072 变成 95,832 字节，三支调度最重的程序也大幅缩小
（`t84_sealed_switch` 521,456 → 113,904、`t133_arrow_blocks` 509,536 → 105,688、
`t51_java25_tour` 523,696 → 438,560），而它们的输出逐字节不变。**会反射的程序不变**：
`t146_reflect` 是 4,712,720 字节，与剪枝前一致，因为反射会从 `main` 抓住每一张成员
表——所以下面「用到反射就带上整份约 3 MB」那句仍然成立，剪枝省下的是不反射的程序的
大小。速度没有可测到的变化：六支 benchmark 交错跑二十轮，每个差异都在噪音内、checksum
全部相同。

还有一点空间没收：95,832 字节对上「每个槽位都填 `NULL`」的底线，差别在于每个还活着
的类都保留了槽位 0、1、2 与四个索引（7、12、13、16，对应 `Class.toString`／
`isArray`／`isInterface`／`isPrimitive`），而不是只留给调度者的子类——生成的调度其实
带着 owner，所以这几个可以再靠一次 `isSubclass` 走访收紧。
