...
|---|---|---|---|---|
| `bench_alloc` | 1.3982 s | 0.2042 s | **6.85 倍时间** | 4,408 kB |
...
| `bench_oop` | 1.0016 s | 0.3572 s | **2.80 倍** | 4,408 kB |
...
**`bench_alloc` 这一行要单独讲。** 它从前写着“靠逃逸分析赢过 HotSpot”：长跑（8 亿次分配）是
**Java 快 6.8 倍**，短跑也已经是 Teyru 慢 1.19 倍。而且它**不是**在量收集器——`TEYRU_GCTRACE=1`
在这条长跑里一次收集都没有打印，因为逃逸分析把那个 `Cell` 留在栈上，这条路径既没有写进堆也
没有触发回收，所以用“GC”解释它的倍数是不对的。
...
一行都没赢——**编译期能拿到的 Teyru 都拿到了，剩下的差距在执行期的分配器与收集器**。
