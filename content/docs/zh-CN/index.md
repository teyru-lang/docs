...
**大小那一行现在不是成绩，是一个未解决的问题——但原因已经知道了。** 它是同一个 hello
world 在 `-O2` 下以 `wc -c` 量的，今天是 501,072 字节（约 489 KB）；同一个程序在
`74fa648`（9/13）是 **48,840 字节**，退步从 `52913a0`（`java.util.function`）开始。
原因是一个结构性的问题，不是哪一支程序写坏了：**vtable 是一串地址，而 LTO 删不掉地址
被取用的函数**（`vt_X[i] = (void*)M_X_i`），`cls_X` 又在每个程序里都是活着的，所以一个
类声明的每个方法都留了下来，每个又指名它分配的类——这个闭包吞掉了大半个标准
库（一支只印一个字符串的 hello world 背着 `java.util.stream`，因为 `String.lines()`
就坐在 `String.length()` 旁边）。量到的：hello world 里有 1,262 个函数存活，其中 951 个
是前缀的方法，而真正被**调用**到的只有 42 个。机制写在
[docs/architecture.md](/zh-CN/docs/architecture) 的〈为什么每个可执行文件都带着前缀〉。

把「没有调用点在用的槽位」剪掉的做法量到 95,064 字节，但它让三支测试
（`t133_arrow_blocks`、`t84_sealed_switch`、`t51_java25_tour`）在一个没被留下的 `NULL`
槽位上 segfault，所以已退回、修正中；编译器仓库的 `AGENTS.md` §10 记着完整的测量。
这一行在剪枝真正进编译器之后才会换成新数字。
