...
同一台机器实测（nobara-pc、AMD Ryzen 7 5700X（8 核 16 线程）、31 GB、linux/amd64、clang 22.1.8、
OpenJDK 21.0.11 Temurin 默认参数；`RUNS=5 JAVA=1 sh scripts/bench.sh`，每行取五次最佳、`-O2`，
数字是**挂钟（wall-clock）的整支程序时间、含进程启动**，两边给同一个规模参数，编译器取自
`130565a`）。脚本现在自己打印方法表头——主机、架构、核心数、树、`RUNS`／`JAVA`／`-O2`，以及
进入与离开时的 1／5／15 分钟负载——所以方法以那段输出为准，这里不重述。

**短跑**（每个程序自己的默认规模；这一组量到的有很大一部分是启动）：

| 程序 | Teyru | Java | Teyru／Java |
|---|---|---|---|
| `bench_alloc` | 0.0371 s | 0.0312 s | 1.19 倍时间 |
| `bench_fib` | 0.0079 s | 0.0277 s | 0.29 倍 |
| `bench_invoke` | 0.4094 s | 0.2517 s | 1.63 倍 |
| `bench_loop` | 0.0249 s | 0.0467 s | 0.53 倍 |
| `bench_oop` | 0.0062 s | 0.0265 s | 0.23 倍 |
| `bench_string` | 0.0144 s | 0.0549 s | 0.26 倍 |
| `bench_string_cjk` | 0.0165 s | 0.0781 s | 两边答案不同，这只是时间 |

**方法的一条注记：控制组要跑。** 这一节有两行曾经把「对方的编译器做了什么」读成自己的成绩或
别人的缺点（一个量到一份编错的构建，一个把 JIT 消掉的分配算成 Java 的分配成本），两次都是
**跑对照组**才看出来的：把 JIT 关掉（`java -Xint -cp <classes> bench_alloc 50000000`）之后，
同一支程序是 1.92 s 而不是 0.03 s。所以这里每一个「快／慢几倍」都要有对照组：换优化等级、
换 C 编译器，或关掉 JIT，看结论还在不在。

**长跑**（每行至少一秒的工作；规模是 fib 44、loop 1000000 轮、oop 500000 轮、string 16000000
次拼接、string_cjk 4000000、alloc 800000000 次分配、invoke 200000000 次调用）：

| 程序 | Teyru | Java | Teyru／Java | 峰值 RSS（Teyru） |
|---|---|---|---|---|
| `bench_alloc` | 1.3982 s | 0.2042 s | **6.85 倍时间**（这一行量的是两件事，见下） | 4,408 kB |
| `bench_fib` | 1.8773 s | 1.7998 s | 1.04 倍（平手） | 4,408 kB |
| `bench_invoke` | 4.0655 s | 1.8586 s | **2.19 倍** | 9,400 kB |
| `bench_loop` | 1.1335 s | 1.0068 s | 1.13 倍 | 4,364 kB |
| `bench_oop` | 1.0016 s | 0.3572 s | **2.80 倍**（见下） | 4,408 kB |
| `bench_string` | 0.7496 s | 0.2848 s | **2.63 倍** | 8,732 kB |
| `bench_string_cjk` | 0.2338 s | 0.1602 s | 两边答案不同 | 8,844 kB |

**不是程序的三个数字：**

| 指标 | Teyru | Java | 差距 |
|---|---|---|---|
| 启动 100 次总时间 | **0.0763 s** | 2.1093 s | **约 27.6 倍快** |
| hello 可执行文件（`-O2`，`wc -c`） | **66,808 B** | — | — |
| 峰值 RSS（hello） | **4,348 kB** | 51,420 kB | **约 11.8 倍省** |

**长跑是唯一能支持吞吐结论的一组，而它说的是：Teyru 一项都没赢。** `fib(44)` 是平手（Java
快 4%），其余每一行都是 Java 快：`loop` 快 1.13 倍、`string` 快 2.6 倍、`oop` 快 2.8 倍、
`alloc` 快 6.8 倍、`invoke` 快 2.2 倍。短跑那一组看起来相反，是因为整支程序的时间里有很大
一部分是 JVM 启动（同一台机器约 20 ms 对 0.77 ms），那是启动的差别，不是吞吐的差别。

**`bench_alloc` 这一行量的是两件不同的事，所以它带的是注解而不是一个数字。** 同一支程序
（`examples/bench_alloc.teyru` 与 `.java` 逐行同形）、五千万次迭代：

| | 时间 | 每次迭代 |
|---|---|---|
| `java`（有 JIT） | 0.03 s | 0.6 ns ← **分配被 JIT 消掉了** |
| `java -Xint`（关掉 JIT） | 1.92 s | 38 ns ← Java 真要付的代价 |
| `teyru` | 0.09 s | 1.8 ns |

每次「分配」0.6 ns 比一次内存写入还快，所以那不是分配——HotSpot 的逃逸分析把整个对象标量化了。
**关掉 JIT 之后，同一支程序在同一规模下 Java 要 1.92 s，Teyru 0.09 s：分配这件事 Teyru 快约
20 倍**，不是表上那个 6.85 倍慢。表上那一行把「Teyru 真的分配」拿去比「Java 把分配优化掉」，
所以右边那一栏的 0.15× 不能读成分配器的胜负。

**要活过两边的优化器，对象必须真的逃逸**（例如进到数组里）——`bench_invoke` 就是那个形状，
所以它的 0.46× 可信。要看「Java 分配一次要付多少」，`-Xint` 那一栏才是诚实的参考。顺带一提，
`TEYRU_GCTRACE=1` 在这条长跑里一次收集都没有打印，因为逃逸分析把那个 `Cell` 留在栈上，
这条路径既没有写进堆也没有触发回收——**所以那一行也不是在量收集器**。

**`bench_oop` 那一行要加一句。** 那些对象进到 `Shape[]` 里，所以不会被消掉，这一行是真的；
但 Java 那一边 `shapes[i].area()` 是只有两个实现类型的调用点，JIT 把两个都内联了，所以这一行
量的是 **Teyru 的调度对上 Java 特化过的调度**。那正是 W14 第 2 项要关掉的差距（单型 inline
cache 或缓存的 itab）；它不该被读成“Java 的接口调用本质上快两倍”。

**峰值 RSS 的旧数字可能是坏的。** harness 先前把程序跑成 `/usr/bin/time -v timeout LIMIT prog`，
量到的是外层的 `timeout`，所以每一笔读数的下限被抬到约 9.9 MB（真的差 12 倍也会打印成
9,904 kB 对 50,832 kB）。今天修好了，上面那一行量的是程序本身，而且取五次**最大**的那一次
（峰值就是最大值，不是最佳值）。

`bench_loop` 的短跑是 0.53 倍而长跑是 1.13 倍，差别同样是启动；两者都保留安全点检查的代价
（每个循环回边一次），那是停止世界（stop-the-world）收集器刻意的设计，说明见
[docs/language.md](/docs/language) §11。

**大小那一行是成绩，而且原因是具体的。** 它是〈快速开始〉那支 hello world
（`System.out.println("Hello, Teyru!")`）在 `-O2` 下以 `wc -c` 量的，今天 **67,240
字节**（约 65.7 KB；上表那一行的 66,808 是 W10 测量当时的树 `130565a`）。这个数字主要是编译器**剪掉没有调用点在用的 vtable 槽位**换来的
...
`ty_stack_check()`）再加 2,256 到 66,688，**W9 的 TLS 链接改动**（`#118`，改由调用图决定要不要链接）再加 120，到 66,808，**W5／W6／W8 这一批**再加 432，到**今天的 67,240**。剪枝省下的是不反射的程序的大小，这两
项买到的是语义与可拦截的错误，两边都不是免费的。

**这个数字要连着优化等级读。** 同一个 hello world 今天是 `-O0` 118,608、`-O1` 90,352、
`-O2` 67,240、`-O3` 70,672 字节；这一行与 `scripts/bench.sh` 用的都是 `-O2`，也是默认值。

前后对照（同一台机器、`-O2`、`wc -c`）：hello world 剪枝前 501,072 → 第一版 95,832 →
第二版 55,920 → 今天 67,240；`t84_sealed_switch` 今天 86,736（剪枝前 521,456、第一版
113,904）、`t133_arrow_blocks` 今天 73,640（509,536、105,688）、`t51_java25_tour` 今天
282,488（523,696、438,560），而且这些程序的输出逐字节不变。**会反射的程序不受剪枝
影响**：`t146_reflect` 今天 5,404,176 字节、`t101_gson` 5,382,816，剪枝对它们没有帮助，
因为反射会从 `main` 抓住每一张成员表——所以用反射的程序仍然要为整份成员表付出代价
（今天约 5.4 MB 而不是 3 MB），剪枝省下的是不反射的程序的大小。速度没有可测到的变化：六支
...
4. **不需要分配的对象就不分配。** 逃逸分析把“不离开所在方法”的对象放在 C 栈上，
   ...
   效果相同。这条路径的证据是 `TEYRU_GCTRACE=1` 下的 `bench_alloc` 长跑**一次收集都
   没有**；但它不使 `bench_alloc` 的总时间赢过 HotSpot（长跑 Java 快 6.8 倍，见上表），
   因为那一行的工作就是分配本身，而 HotSpot 的 TLAB bump 与分代回收把它做得更快。
...
交给其他对象的对象仍然走堆与标记清除回收，而 HotSpot 有分代假设，所以“对象长期存活、
反复回收”的负载是 JVM 的强项。这不是预测：上面那张长跑表就是那个负载，而六行里 Teyru
一行都没赢（`bench_alloc` 那一行见上，它量的是两件事）——**编译期能拿到的 Teyru 都拿到了，
剩下的差距在执行期的分配器与收集器**。
短跑那一组的优势来自启动，不是吞吐；重现方式见 `RUNS=5 JAVA=1 sh scripts/bench.sh`，
七支 benchmark 程序、启动 100 次、可执行文件大小与峰值内存都由这支脚本测量（取最佳，
在上面那台机器上执行）。
...
| 路径 | 说明 |
|---|---|
| `cmd/teyru` | CLI 入口（`build`／`run`／`emit`／`emit-llvm`／`emit-java`／`get`／`mod`／`version`） |
...
- **字符串**：`tystr { tyobj obj; int64_t blen; int32_t ulen; uint32_t flags }`——**WTF-8**
  ...
  首次用到才建，所以没有索引过的字符串不必付这个成本）。**`String.length()` 数的是 `ulen`**
  （UTF-16 code unit；`getBytes().length` 才是 `blen` 的字节数，两者在 U+007F 以上分开）；
  字面量是静态对象，不经过 GC。
...
1. **分号可省。** 语句以换行结束，分号会被忽略（W7 之后；`TY-SYN-0001` 已移除），所以未经修改的
   `.java` 可以直接编译。
...
teyru emit-java [flags] <files...>             打印等效的 Java 源代码
...
| `--cc-flag <arg>` | 传给 C 编译步骤的参数（可重复），例如用 `-I <dir>` 把 `--native-header` 的输出放进 include path |
...
**256 支**（测试仓库的 `34584f2`）扫过一轮（`teyru build --backend=llvm`，建得起来的每一支都跑起来与 `.expected`
...
| `linux/arm64` | ✅ 用 `aarch64-linux-gnu-gcc` 交叉编译；那个目标的 sysroot 是另外装上去的（见下） | ✅ 在 qemu-aarch64 下跑完整套件（W9 之前那一对：**250 项全过**）。W9 之后用 `TEYRU_TARGET=linux/arm64` 重测：**286 项里 271 过、5 失败、10 已知失败**；那 5 项**每一项在原生 amd64 上也同样失败**（逐项验过），所以没有一项是 arm64 的问题（见下） |
...
（见下），`linux/arm64` 那一行现在两个测量都有：W9 之前的 250 项全过，以及 W9 之后用
`TEYRU_TARGET` 跑完的 286 项（271 过、5 失败——每一项原生也失败，见下）。
...
那次跑的是**没有改过的 `tests/run.sh`**，而且只有三件事由外面给。W9 之后不必再那样绕：`run.sh` 与 `go test` 都读 `TEYRU_TARGET`，目标由那个变量给，`.skip` 也照那个平台判读；当时的做法是把编译器多包一层 `--target linux/arm64`（`run.sh` 只喊 `teyru build -O1 -o <输出> <来源>`，目标由编译器那个名字带着走）。`CC` 是
...
QEMU_LD_PREFIX=/usr/aarch64-linux-gnu/sys-root sh run.sh`，不用编译器包装），而且已经跑完：
**286 个案例里 271 过、5 失败、10 已知失败、0 跳过**（`tests` @ `e4268a6`、编译器 `5ac017b`，
2026-09-17）。W9 之前在 arm64 上**根本建不起来**的三支——`t138_request_mapping_forms`、
`t140_json_binding_edges`、`t141_web_param_errors`——现在都是 PASS。

**那 5 个失败没有一个是 arm64 的问题，而且每一项都在同一台机器上原生验过：** `t226`／`t227`／
`t241` 三支要的是 `RawClient.ask(String)`，那是 W5／W7 才带进来的 API（原生 `teyru build` 一样以
`TY-TYP-0076` 拒绝）；`t240_progen_array_bounds` 是**那个编译器**的消息用小写 `index ...`，而
`tests` 的期望值是 JDK 的大写 `Index ...`（那条修正在 main 上，W6）：用 main 重编同一支程序，
amd64 与 arm64 都打印 `Index 7 out of bounds for length 3`、两边都过；`native/net_c_test` 是那支 C 测试在
链接时找不到 `TY_UNSUP`／`ty_make_ex`／`ty_throw`（原生编译同样的 `undefined reference`）。
10 个已知失败是 `known-failures.txt` 列出的 W5／W7 项目（`t196`、`t230`–`t239` 的探针），它们
失败是预期的、而且被记录，不是被忽略。
...
`undefined reference to SSL_CTX_new`。要注意「碰得到」算的是**可达性**，而 W9 之后它算的是**程序自己的调用图**：反射用的成员表与 `Class.forName` 的类表不再被当成可达（emitter 标记那些行，TLS 的不动点不走它们），所以 `t146_reflect`、`t101_gson` 与 `t102_web`（Spring 形状的那一层会扫描类）现在都建得起来——`t101_gson` 的 PE32+ 只 import `KERNEL32.dll`、`WS2_32.dll`、`msvcrt.dll`，在 Wine 下的输出与 `.expected` 相同。通过反射走到 TLS 的调用由 `tyrt_net.c` 的弱符号回答一个具名、可拦截的 `UnsupportedOperationException`（`Net.tlsClientContext0: this program was not linked against OpenSSL`），而不是跳到 `NULL`。不用 TLS 的程序不会被链接 OpenSSL。
...
这张表**不是「每一列都跑过」的承诺**。今天缺的那一格只有 macOS 的**运行**：它编得出来（`--cc` 指到一个跑在这里、目标是 macOS 的编译器），但没有 macOS 能运行它，所以那一格是 ❌。W9 收掉了两个「还没解的空隙」：`tests/run.sh` 与 `go test` 读 `TEYRU_TARGET`，目标不必再靠替换编译器名字；`resolveTarget` 也接受调用端给的 `--cc`，所以 Apple 那两列不必再绕过目标表。
...
**默认等级就是使用者实际拿到的等级。** `run` 默认 `-O0`，所以第一次按下去拿到的是 `-O0` 的行为，
而等级之间的差距是真的：常数除数特化（整数除法／取模的除数是常数时直接用乘加）在 `-O0` 值
**1.55×**、`-O1` 值 **1.39×**、`-O2` **不变**——同一支 `bench_loop` 长跑（1,000,000 轮）在 `d44f3fb`
与它的前一个 commit 上各建一次量出来的。**这不是说性能那一节的长跑表变了**：表上那一行是 `-O2`，
而 `-O2` 的前后一模一样（1.16 s 对 1.16 s）。
