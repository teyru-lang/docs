...
| `ty_str_len(tystr *s)` | **UTF-16 code unit** 数（`ulen`），也就是 `String.length()` 回的那个数；要在原生代码里走字节时用的是 `ty_str_blen(s)`（`blen`）。存储是 WTF-8、字节接在头后面（`TY_STR_DATA(s)`，没有第二个指针），见〈语言参考〉§12 第 12 条。没有伙伴的代理是三个字节的形状，而 `getBytes`／`println` 会把它写成一个 `?` |
