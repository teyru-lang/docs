...
### 產生的 C 不得依賴 C 沒有指定的順序

Java 指定運算元與引數**由左而右**求值；C 沒有——函式引數的求值順序是未指定的。所以一個把 Java
運算元直接當成 C 引數傳出去的後端，等於把語意交給編譯器決定。**規則因此是：產生的 C 不得依賴
任何 C 留下未指定的順序**；需要那個順序時就用暫存變數把它固定下來（同一套機制就是上面
`new Foo(...)` 那個 GNU 敘述運算式要的東西）。

這條規則是被**兩個 C 編譯器**逼出來的，而不是被想出來的：同一支程式

```teyru
two(f(1), f(2))     // f 會印出自己被呼叫的次序
```

在 clang 下是 `1(1)2(2)`（與 javac 相同），在 **gcc 下是 `2(1)1(2)`**——答案是錯的，不是崩潰，
而且 `-O0`／`-O1`／`-O2` 都一樣，所以那不是最佳化造成的。這就是後端矩陣同時跑 clang 與 gcc 的
理由：**只跑一個編譯器，這一類缺陷看不到**。它看到的規模不是一支程式：矩陣逐支比對 clang 與 gcc
的 stdout，**有 17 支程式在 clang 下與期望相同、在 gcc 下答案不同**。
**在 `3cd9c70`（v0.4.0 的 tag）上這條規則已經由 C 後端強制**：同一支程式在 clang 與 gcc 下都印
`1(1)2(2)=3`（修好之前 gcc 印的是 `2(1)1(2)`），矩陣會繼續把它釘住。
**LLVM 後端有它自己的待辦**：`EvalOrder` 量到 `--backend llvm -O0` 有三行與 JDK 不同；它不是
`make ci` 的腿，所以不設閘門，列為新的工作項。
...
執行期對作業系統的每一項需求都收在 `internal/runtime/src/tyrt_plat.h` 裡，四十二個
...
| 類別初始化 | `codegen.clinitStmt` | 惰性初始化，但旗標由產生的程式碼自己測：初始化區塊在類別自己的監視器下執行，完成旗標只在它返回之後才發布（JLS 12.4.2），所以另一條執行緒在初始化進行中碰到這個類別會等它，而不是讀到還沒賦值的靜態欄位；繼承鏈上沒有靜態初始化區塊的類別不會有 `<clinit>` 函式（slot 是 `NULL`），`main` 也不會點名它——點名等於在 `main` 裡取它的位址，而一個位址就足以讓連結期最佳化把整個類別連同它的 vtable、介面表與所有方法保留在執行檔裡 |
