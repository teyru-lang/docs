...
**但這四行是「手跑綠」，不是「工作流綠」——這一版兩者不是同一件事。** 工作流程在同一棵樹
`3cd9c70` 上的**首跑失敗**在兩支時區測試（`TestPrograms/t189_timezone_lookup`、
`t190_timezone_tzif`）：**負 DST** 的時區（`Europe/Dublin` 的標準時間是 +01、冬天是 GMT 而
`isdst=1`；`Africa/Casablanca` 同理）在這台主機上綠、在 runner 較新的 **宿主 tzdata** 上紅——
差別不在程式碼，而在宿主 **tzdata 的建構佈局**：vanguard（本機）與 rearguard（Debian／Ubuntu）
對愛爾蘭給出**相同的 offset 與縮寫、相反的 `isdst`**，所以**錯的是測試，不是讀取程式**——沒有
任何一個固定的期望值能同時滿足兩種佈局。修法是讓測試自己帶資料（`t189`／`t190`），
`lib/46_timezone.teyru` **一行未改**；這件事走 `v0.4.1`，而它的說明會寫成「**套件以前會因為
機器的 tzdata 佈局而過或不過**」，不是「修好了一個時區缺陷」。
...
**三條已知失敗，具名。** 「3 已知失敗」不該是一個裝飾用的數字，所以三條都寫出來（括號裡是
`known-failures.txt` 那一行的原因）：

- `t235_probe_exceptions`（W6：類名已經對上 `java.lang.*`，剩下的是轉型的括號註記、有幫助的
  `NullPointerException` 訊息，與 `ArrayStoreException` 的元素類別）
- `t237_probe_missing_api`（W6：`new Integer(int)` 與 `new Boolean(boolean)` 沒有宣告；這支探針
  另外要的字串 API 已經隨 W5 的寫入那一半到位）
- `t239_probe_negative_array`（W7：負的陣列大小在這裡是編譯錯誤，在 Java 是執行期例外）

那個檔案自己的規則讓這份名單有牙齒：**列出的案例如果通過，整次跑就失敗——所以一個條目活不過
它描述的那個缺陷**。所以這三行同時是進度表（每一條都掛著工作項）與承諾（哪一條修好，它就必須
從檔案裡消失）。
