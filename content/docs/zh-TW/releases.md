...
的行為；**進行中**的項目附上 PR 或工作項，合入之後才拿掉標記。發佈時整頁不該再有「進行中」。
...
是 1002／1009／1001。全部十三個 `server.teyru.*` 設定鍵與預設值見
...
- **`HashMap`／`HashSet` 的迭代順序**對齊 JDK 21 與**例外的全限定名**（`java.lang.*`）仍在
  進行中；在那之前它們列在 §13。
...
目標是 {C＋clang、C＋gcc、LLVM} × {`-O0`、`-O2`} 的矩陣全部一致，並把數值提升、複合賦值、
移位、字串串接、裝箱與檢查等規則下沉成共享的降階。LLVM 後端目前的邊界（2026-09-17 實測：
256 支測試程式裡 159 支建得起來、其中 153 支輸出與期望完全相同、93 支被具名拒絕、4 支在
後端之前就被拒絕、0 個模組 clang 不收）寫在 [docs/index.md](/docs) 的〈後端與平台〉。
...
與尖峰 RSS。這一頁不重述效能數字，因為那一頁的數字必須連著量測方法讀：見
[docs/index.md](/docs) 的〈為什麼比 JVM 快〉。
...
| 量到的東西 | 數字 | 怎麼量的 |
|---|---|---|
| 執行檔大小（hello world，`-O2`） | **66,688 B**（`-O1` 85,320、`-O3` 70,128） | `wc -c`；比 0.2 時代的 55,920 大，兩次成長分別是裝箱快取（+6,016）與堆疊檢查（+2,256），量在 [docs/index.md](/docs) |
| LLVM 後端的邊界 | 256 支裡 159 支建得起來、153 支輸出相同、93 支具名拒絕 | 2026-09-17，`teyru build --backend=llvm` 逐支跑並與 `.expected` 比（[docs/index.md](/docs) 有完整分類） |
...
- **Java 原始碼不改就能編**（在 W7 合入並有 `tests/java-compat/` 撐著之前）。
