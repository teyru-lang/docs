...
|---|---|---|---|---|
| `bench_alloc` | 1.3982 s | 0.2042 s | **6.85 倍時間** | 4,408 kB |
...
**`bench_alloc` 這一列要單獨講。** 它從前寫著「靠逃逸分析贏過 HotSpot」：長跑（8 億次配置）
是 **Java 快 6.8 倍**，短跑也已經是 Teyru 慢 1.19 倍。而且它**不是**在量收集器——
`TEYRU_GCTRACE=1` 在這條長跑裡一次收集都沒有印出來，因為逃逸分析把那個 `Cell` 留在堆疊上，
這條路徑既沒有寫進堆積也沒有觸發回收，所以用「GC」解釋它的倍數是錯的。
...
那個負載，而六列裡 Teyru 一列都沒贏——**編譯期能拿到的東西 Teyru 都拿到了，剩下的差距
