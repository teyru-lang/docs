...
同一台機器實測（nobara-pc、AMD Ryzen 7 5700X（8 核 16 緒）、31 GB、linux/amd64、clang 22.1.8、
OpenJDK 21.0.11 Temurin 預設參數；`RUNS=5 JAVA=1 sh scripts/bench.sh`，每列取五次最佳、`-O2`，
數字是**牆鐘的整支程式時間、含 process 啟動**，兩邊給同一個規模參數，編譯器取自 `130565a`）。
腳本現在自己印方法表頭——主機、架構、核心數、樹、`RUNS`／`JAVA`／`-O2`，以及進入與離開時的
1／5／15 分鐘負載——所以方法以那段輸出為準，這裡不重述。

**短跑**（每個程式自己的預設規模；這一組量到的有很大一部分是啟動）：

| 程式 | Teyru | Java | Teyru／Java |
|---|---|---|---|
| `bench_alloc` | 0.0371 s | 0.0312 s | 1.19 倍時間 |
...
| `bench_string_cjk` | 0.0165 s | 0.0781 s | 兩邊答案不同，這只是時間 |

**方法的一則註記：控制組要跑。** 這一節有兩列曾經把「對手的編譯器做了什麼」讀成自己的成績或
別人的缺點（一個是量到一份編錯的建置，一個是把 JIT 消掉的配置算成 Java 的配置成本），兩次都是
**跑對照**才看出來的：把 JIT 關掉（`java -Xint -cp <classes> bench_alloc 50000000`）之後，
同一支程式是 1.92 s 而不是 0.03 s。所以這裡的每個「快／慢幾倍」都要有控制組：換最佳化等級、
換 C 編譯器、或關掉 JIT，看結論還在不在。

**長跑**（每列至少一秒的工作；規模是 fib 44、loop 1000000 輪、oop 500000 輪、string 16000000
次串接、string_cjk 4000000、alloc 800000000 次配置、invoke 200000000 次呼叫）：

| 程式 | Teyru | Java | Teyru／Java | 尖峰 RSS（Teyru） |
|---|---|---|---|---|
| `bench_alloc` | 1.3982 s | 0.2042 s | **6.85 倍時間**（這一列量的是兩件事，見下） | 4,408 kB |
...
| `bench_oop` | 1.0016 s | 0.3572 s | **2.80 倍**（見下） | 4,408 kB |
| `bench_string` | 0.7496 s | 0.2848 s | **2.63 倍** | 8,732 kB |
| `bench_string_cjk` | 0.2338 s | 0.1602 s | 兩邊答案不同 | 8,844 kB |

**不是程式的三個數字：**

| 指標 | Teyru | Java | 差距 |
|---|---|---|---|
| 啟動 100 次總時間 | **0.0763 s** | 2.1093 s | **約 27.6 倍快** |
| hello 執行檔（`-O2`，`wc -c`） | **66,808 B** | — | — |
| 尖峰 RSS（hello） | **4,348 kB** | 51,420 kB | **約 11.8 倍省** |

**長跑是唯一能支持吞吐結論的一組，而它說的是：Teyru 一項都沒贏。** `fib(44)` 是平手（Java
快 4%），其餘每一列都是 Java 快：`loop` 快 1.13 倍、`string` 快 2.6 倍、`oop` 快 2.8 倍、
`alloc` 快 6.8 倍、`invoke` 快 2.2 倍。短跑那一組看起來相反，是因為整支程式的時間有很大一
部分是 JVM 啟動（同一台機器約 20 ms 對 0.77 ms），那是啟動的差別，不是吞吐的差別。

**`bench_alloc` 這一列量的是兩件不同的事，所以它帶的是註解而不是一個數字。** 同一支程式
（`examples/bench_alloc.teyru` 與 `.java` 逐行同形）、五千萬次迭代：

| | 時間 | 每次迭代 |
|---|---|---|
| `java`（有 JIT） | 0.03 s | 0.6 ns ← **配置被 JIT 消掉了** |
| `java -Xint`（關掉 JIT） | 1.92 s | 38 ns ← Java 真的要付的代價 |
| `teyru` | 0.09 s | 1.8 ns |

每次「配置」0.6 ns 比一次記憶體寫入還快，所以那不是在配置——HotSpot 的逃逸分析把整個物件
純量化了。**關掉 JIT 之後，同一支程式在同一規模下 Java 要 1.92 s，Teyru 0.09 s：配置這件事
Teyru 快約 20 倍**，不是表上那個 6.85 倍慢。表上那一列把「Teyru 真的配置」拿去比「Java 把配置
最佳化掉」，所以右邊那一欄的 0.15× 不能讀成配置器的勝負。

**要活過兩邊的最佳化器，物件必須真的逃逸**（例如進到陣列裡）——`bench_invoke` 就是那個形狀，
所以它的 0.46× 可信。要看「Java 配置一次要付多少」，`-Xint` 那一欄才是誠實的參考。順帶一提，
`TEYRU_GCTRACE=1` 在這條長跑裡一次收集都沒有印出來，因為逃逸分析把那個 `Cell` 留在堆疊上，
這條路徑既沒有寫進堆積也沒有觸發收集——**所以那一列也不是在量收集器**。

**`bench_oop` 那一列要加一句。** 那些物件進到 `Shape[]` 裡，所以不會被消掉，這一列是真的；
但 Java 那一邊 `shapes[i].area()` 是只有兩個實作型別的呼叫點，JIT 把兩個都內聯了，所以這一列
量的是 **Teyru 的調度對上 Java 特化過的調度**。那正是 W14 第 2 項要關掉的差距（單型 inline
cache 或快取的 itab）；它不該被讀成「Java 的介面呼叫本質上快兩倍」。

**尖峰 RSS 的舊數字可能是壞的。** harness 先前把程式跑成 `/usr/bin/time -v timeout LIMIT prog`，
量到的是外層的 `timeout`，所以每一筆讀數的下限被抬到約 9.9 MB（真的差 12 倍也會印成
9,904 kB 對 50,832 kB）。今天修好了，上面那一列量的是程式本身，而且取五次**最大**的那一次
（尖峰就是最大值，不是最佳值）。

`bench_loop` 的短跑是 0.53 倍而長跑是 1.13 倍，差別同樣是啟動；兩者都保留安全點檢查的代價
（每個迴圈回邊一次），那是停止世界（stop-the-world）收集器刻意的設計，說明見
...
位元組**（約 65.7 KB；上表那一列的 66,808 是 W10 量測當時的樹 `130565a`）。這個數字主要是編譯器**剪掉沒有呼叫點在用的 vtable 槽位**換來的
...
`ty_stack_check()`）再加 2,256 到 66,688，**W9 的 TLS 連結改動**（`#118`，改由呼叫圖決定要不要連結）再加 120，到 66,808，
**W5／W6／W8 這一批**再加 432，到**今天的 67,240**。剪枝省下的是不反射的程式的大小，這兩
項買到的是語意與可攔截的錯誤，兩邊都不是免費的。

**這個數字要連著最佳化等級讀。** 同一支 hello world 今天是 `-O0` 118,608、`-O1` 90,352、
`-O2` 67,240、`-O3` 70,672 位元組；這一列與 `scripts/bench.sh` 用的都是 `-O2`，也是預設值。
...
282,488（523,696、438,560），而且這些程式的輸出逐位元組不變。**會反射的程式不受剪枝
影響**：`t146_reflect` 今天 5,404,176 位元組、`t101_gson` 5,382,816，剪枝對它們沒有幫助，
因為反射會從 `main` 抓住每一張成員表——所以用反射的程式仍然要為整份成員表付出代價
（今天約 5.4 MB 而不是 3 MB），剪枝省下的是不反射的程式的大小。速度沒有可測到的變化：六支
...
4. **不需要配置的物件就不配置。** 逃逸分析把「不離開所在方法」的物件放在 C 堆疊上，
   ...
   同樣的效果。這條路徑的證據是 `TEYRU_GCTRACE=1` 下的 `bench_alloc` 長跑**一次收集
   都沒有**；但它不使 `bench_alloc` 的總時間贏過 HotSpot（長跑 Java 快 6.8 倍，見上表），
   因為那一列的工作是配置本身，而 HotSpot 的 TLAB bump 與分代回收把它做得更快。
...
所以「物件長期存活、反覆回收」的負載是 JVM 的強項。這不是預測：上面那張長跑表就是
那個負載，而六列裡 Teyru 一列都沒贏（`bench_alloc` 那一列見上，它量的是兩件事）——**編譯期能
拿到的東西 Teyru 都拿到了，剩下的差距
在執行期的配置器與收集器**。短跑那一組的優勢來自啟動，不是吞吐；重現方式見
`RUNS=5 JAVA=1 sh scripts/bench.sh`，七支 benchmark 程式、啟動 100 次、執行檔大小與
尖峰記憶體都由這支腳本量測（取最佳，於上面那台機器上執行）。
...
注意：**Teyru 的敘述不需要分號**——每個敘述以換行結束，而 W7 之後寫了分號也不報錯
（這是未修改的 `.java` 檔能直接當輸入的原因之一）；`for` 標頭用兩個冒號分隔三段。
...
**在測過的那個子集上，未修改的 Java 原始碼現在編得過**（W7）：`.teyru` 的分號可省、`.java`
...
**256 支**（測試倉庫的 `34584f2`）掃過一輪（`teyru build --backend=llvm`，建得起來的每一支都跑起來與 `.expected`
...
**這是怎麼量的，以及為什麼兩個數字都要說清楚。** 2026-09-17、`zig 0.16.0`、`zig cc -target aarch64-macos` 包成 `--cc`、`-O1`、一次 4 支（`tests` @ `e4268a6`，編譯器取自 W9 落地的 `5ac017b`）。257 支裡 **240 支建得起來並連結**；**9 支由驅動具名拒絕**（`t154_http_client`、`t162_http_roundtrip`、`t163_https_roundtrip`、`t191_tls_keepalive`、`t192_tls_handshake_timeout`、`t207_http_server_certificate`、`t210_http_gzip_edges`、`t212_stream_end_of_stream`、`t225_http_serve_loop`）；剩下 **8 支不是 darwin 的失敗**，是那個版本的編譯器還不接受的程式（`t180_http_gzip`、`t196_string_bytes`、`t226`／`t227`／`t241`、`t237`／`t238`／`t239`——W5／W7 的 API 與檢查器），換成今天 main 的編譯器重跑應該會少掉那 8 支。
**`darwin/amd64` 是把同一支腳本的目標換成 `x86_64-macos` 再跑一次**（同樣 P=4、`-O1`、同樣的
編譯器與 `tests`），三個數字一模一樣，具名拒絕的 9 支與編不過的 8 支也是同一批程式——所以
那兩列共用一句話，而不是各有各的說法。**沒有任何一支被執行過。**

W9 之前那條繞道（`teyru emit` 的 C 加上執行期六個檔案交給 `zig cc`）與它量到的兩個數字（不碰 TLS 的 188 支全部編譯並連結成功、碰得到 TLS 的 34 支不行）在這裡保留為歷史，而且兩半都過時了：188 是 222 支程式時的數字，34 是**舊規則**下的碰得到 TLS（任何會反射的程式都算），W9 之後算的是程式自己的呼叫圖，所以那 34 支現在多數建得起來，剩下的就是上面那 9 支。
...
release 上，跑的內容與人跑的是同一組（`make ci`、`make java-compat`、`make jdk-diff`、
`make backend-matrix`、`make notices`、`tests/run.sh`），push 與 PR 都不會觸發它。arm64 因此不再停在「實作了、沒有任何人跑過」
...
- `scripts/backend-matrix-allow.txt`（在 Teyru 倉庫）：後端矩陣**允許**的跨格差異，一行一個
  `<程式> <工作項> <原因>`。它與 `known-failures.txt` 的機制相同，不同的是它原諒的是
  **兩個後端彼此不一致**——那是套件自己的 driver 看不到的，因為它們只建一個後端。
...
**預設等級就是使用者實際拿到的等級。** `run` 預設 `-O0`，所以第一次按下去拿到的是 `-O0` 的行為，
而等級之間的差距是真的：常數除數特化（整數除法／取模的除數是常數時直接用乘加）在 `-O0` 值
**1.55×**、`-O1` 值 **1.39×**、`-O2` **不變**——同一支 `bench_loop` 長跑（1,000,000 輪）在 `d44f3fb`
與它的前一個 commit 上各建一次量出來的。**這不是說效能那一節的長跑表變了**：表上那一列是 `-O2`，
而 `-O2` 的前後一模一樣（1.16 s 對 1.16 s）。
