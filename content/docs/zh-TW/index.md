...
**大小那一列現在不是成績，是一個未解的問題——但原因已經知道了。** 它是同一個 hello
world 在 `-O2` 下以 `wc -c` 量的，今天是 501,072 位元組（約 489 KB）；同一個程式在
`74fa648`（9/13）是 **48,840 位元組**，退步從 `52913a0`（`java.util.function`）開始。
原因是一個結構性的問題，不是哪一支程式寫壞：**vtable 是一串位址，而 LTO 刪不掉位址
被取用的函式**（`vt_X[i] = (void*)M_X_i`），`cls_X` 又在每個程式裡都是活著的，所以一個
類別宣告的每個方法都留了下來，每個又指名它配置的類別——這個閉包吞掉了大半個標準
程式庫（一支只印一個字串的 hello world 背著 `java.util.stream`，因為 `String.lines()`
就坐在 `String.length()` 旁邊）。量到的：hello world 裡有 1,262 個函式存活，其中 951 個
是前綴的方法，而真正被**呼叫**到的只有 42 個。機制寫在
[docs/architecture.md](/docs/architecture) 的〈為什麼每個執行檔都帶著前綴〉。

把「沒有呼叫點在用的槽位」剪掉的做法量到 95,064 位元組，但它讓三支測試
（`t133_arrow_blocks`、`t84_sealed_switch`、`t51_java25_tour`）在一個沒被留下的 `NULL`
槽位上 segfault，所以已退回、修正中；編譯器倉庫的 `AGENTS.md` §10 記著完整的量測。
這一列在剪枝真正進編譯器之後才會換成新數字。
