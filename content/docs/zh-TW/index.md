...
量的，今天 95,832 位元組（約 93.6 KB）。這個數字來自編譯器**剪掉沒有呼叫點在用的
vtable 槽位**——機制寫在 [docs/architecture.md](/docs/architecture) 的〈為什麼每個執行檔
都帶著前綴〉。兩天前它不是這樣：`74fa648`（9/13）是 48,840 位元組，之後一路長到
501,072，因為 C 後端把每個方法都寫出來、把「沒人呼叫的東西」交給 clang 的 LTO 刪，
而 LTO 刪不掉位址被取用的函式——vtable 就是一串位址——於是 hello world 背著大半個標準
程式庫（1,262 個函式存活，其中 951 個是前綴的方法，真正被呼叫到的只有 42 個）。

剪枝之後：hello world 從 501,072 變成 95,832 位元組，三支調度最重的程式也大幅縮小
（`t84_sealed_switch` 521,456 → 113,904、`t133_arrow_blocks` 509,536 → 105,688、
`t51_java25_tour` 523,696 → 438,560），而它們的輸出逐位元組不變。**會反射的程式不變**：
`t146_reflect` 是 4,712,720 位元組，與剪枝前一致，因為反射會從 `main` 抓住每一張成員
表——所以下面「用到反射就帶上整份約 3 MB」那句仍然成立，剪枝省下的是不反射的程式的
大小。速度沒有可測到的變化：六支 benchmark 交錯跑二十輪，每個差異都在噪音內、checksum
全部相同。

還有一點空間沒收：95,832 位元組對上「每個槽位都填 `NULL`」的底線，差別在於每個還活著
的類別都保留了槽位 0、1、2 與四個索引（7、12、13、16，對應 `Class.toString`／
`isArray`／`isInterface`／`isPrimitive`），而不是只留給調度者的子類別——產生的調度其實
帶著 owner，所以這幾個可以再靠一次 `isSubclass` 走訪收緊。
