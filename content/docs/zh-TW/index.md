...
位元組**（約 65.2 KB）。這個數字主要是編譯器**剪掉沒有呼叫點在用的 vtable 槽位**換來的
...
`ty_stack_check()`）再加 2,256 到 66,688，**W9 的 TLS 連結改動**（`#118`，改由呼叫圖決定要不要連結）再加 120，到**今天的 66,808**。剪枝省下的是不反射的程式的大小，這兩
項買到的是語意與可攔截的錯誤，兩邊都不是免費的。

**這個數字要連著最佳化等級讀。** 同一支 hello world 今天是 `-O1` 85,440、`-O2` 66,808、
`-O3` 70,248 位元組；這一列與 `scripts/bench.sh` 用的都是 `-O2`，也是預設值。
