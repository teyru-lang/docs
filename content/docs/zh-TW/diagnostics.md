...
`TY-SYN-0002`、`0004`–`0011` 由詞法分析器產生（`TY-SYN-0001` 已移除，見下表；`TY-SYN-0003` 例外：它是剖析器
...
| 代碼 | 訊息 | 說明與修法 |
|---|---|---|
| TY-SYN-0001 | （已移除） | 分號曾經是錯誤；W7 之後分號被忽略（見〈語言參考〉§1），這個碼不再由編譯器產生。 |
...
| TY-INT-0100 | `<建構>: <原因>, in <位置>`（例：`an inner class (Main$Local$2): the llvm back end does not lower the enclosing-instance chain, in Main.instanceLocal`） | `--backend=llvm` 對它降不下去的建構具名拒絕，訊息說出碰到的是什麼、為什麼、在哪裡，而不是安靜地退回 C 後端。要用 C 後端就不要加 `--backend=llvm` |
| TY-INT-0101 | `the llvm back end compiles for linux/amd64 only (%s was asked for); use the c back end for other platforms` | LLVM 後端只編 linux/amd64；其他目標請用 C 後端 |
| TY-INT-0102 | `cannot emit Java for <特性>: <原因>`（例：`cannot emit Java for the native property years: Java has no property syntax: …`） | `teyru emit-java` 遇到寫不成 Java 的 Teyru 特性時拒絕，訊息指名那個特性與原因。印出來的 Java 要能編就改寫那個特性 |
...
| `ArrayIndexOutOfBoundsException` | 陣列索引超出 `[0, length)`（讀與寫都是；`null` 陣列先丟 `NullPointerException`）；`Arrays.copyOfRange` 那類範圍引數越界也用它 |
| `StringIndexOutOfBoundsException` | 字串與 `StringBuilder` 的索引或範圍超出：`charAt`、`substring`、`delete`、`StringBuilder.insert` 等（`String` 的每一個邊界檢查都是這一個） |
...
越界那三個的繼承階層與 Java 相同：`IndexOutOfBoundsException` 是父類別，
`ArrayIndexOutOfBoundsException` 與 `StringIndexOutOfBoundsException` 在它底下，所以
`catch (IndexOutOfBoundsException e)` 接得到三種。這也是 Java 程式碼裡最常見的寫法，
它在這裡與在 Java 一樣有效（見 [docs/language.md](/docs/language) §10）。
